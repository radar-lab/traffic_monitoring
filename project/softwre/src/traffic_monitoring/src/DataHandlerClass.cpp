...
DataUARTHandler::DataUARTHandler(ros::NodeHandle* nh) : currentBufp(&pingPongBuffers[0]) , nextBufp(&pingPongBuffers[1]) {
    //DataUARTHandler_pub = nh->advertise<sensor_msgs::PointCloud2>("/traffic_monitoring/radar_scan_pcl", 100);
    radar_scan_pub = nh->advertise<traffic_monitoring::RadarScan>("/traffic_monitoring/radar_scan", 100);
    marker_pub = nh->advertise<visualization_msgs::Marker>("/traffic_monitoring/radar_scan_markers", 100);
    ...
    // Wait for parameters
    while(!nh->hasParam("/traffic_monitoring/doppler_vel_resolution")){}

    nh->getParam("/traffic_monitoring/numAdcSamples", nr);
    nh->getParam("/traffic_monitoring/numLoops", nd);
    nh->getParam("/traffic_monitoring/num_TX", ntx);
    nh->getParam("/traffic_monitoring/f_s", fs);
    nh->getParam("/traffic_monitoring/f_c", fc);
    nh->getParam("/traffic_monitoring/fc_chirp", fc_chirp);
    nh->getParam("/traffic_monitoring/BW", BW);
    nh->getParam("/traffic_monitoring/PRI", PRI);
    nh->getParam("/traffic_monitoring/t_fr", tfr);
    nh->getParam("/traffic_monitoring/max_range", max_range);
    nh->getParam("/traffic_monitoring/range_resolution", vrange);
    nh->getParam("/traffic_monitoring/max_doppler_vel", max_vel);
    nh->getParam("/traffic_monitoring/doppler_vel_resolution", vvel);
...
void *DataUARTHandler::sortIncomingData( void )
{
    ...
    traffic_monitoring::RadarScan *radarscan_cf = NULL; // current frame
    traffic_monitoring::RadarScan radarscan_pub; // last frame
    ...
    while(ros::ok())
    {
        ...
        case READ_POINT_CLOUD_2D:
            ...
            if(radarscan_cf == NULL)
            {
                radarscan_cf = (traffic_monitoring::RadarScan * )malloc(num_Points * sizeof(traffic_monitoring::RadarScan));
            }
            //num_Points = (tlvLen-sizeof(mmwData.tlvHeader))/sizeof(mmwData.PointCloudData);
            for(i=0; i<num_Points; i++)
            {
                // Refer to C:\ti\mmwave_industrial_toolbox_4_0_0\labs\traffic_monitoring\18xx_68xx_traffic_monitoring\gui\getGtrackPtCloud.m
                //get range
                ...
                radarscan_cf[i].y           = -point_range_xy_plane * sin(radarscan_cf[i].angle);
                radarscan_cf[i].x           = point_range_xy_plane * cos(radarscan_cf[i].angle);
        ...
        case READ_TARGET_INDEX: 
            ...
            if(Point_List_Idx == 4){
                ...
                for(i=0; i<num_TargetIdx; i++)
                {
                    radarscan_pub.header.frame_id       = "traffic_monitoring_radar";
...
void DataUARTHandler::visualize(const traffic_monitoring::RadarScan &msg){
    visualization_msgs::Marker marker;
    marker.header.frame_id = "traffic_monitoring_markers";
